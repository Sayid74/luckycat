...
public class Mimes
{
    ...
    static {
        mimeTypeMapping = new HashMap<String, String>() {{
            ...
            put("shtml"   , MIME_TEXT_HTML);
            put("shtm"    , MIME_TEXT_HTML);
            put("jsp"     , MIME_TEXT_HTML);
            put("asp"     , MIME_TEXT_HTML);
    ...
    /**
     * Guss is the page a html format by content type.
     * @param contentType 页面的 content-type 属性。
     * @return 返回值为“真”时，说明内容确实是 html 页面。
     */
    public static boolean isHTMLPage(String contentType)
    {
        return (contentType != null && !contentType.isEmpty())
            && "text/html".equals(contentType.trim().toLowerCase());
    }

    /**
     * 通过ContentType 判断一个文件是不是 css 文件。
     * @param contentType 页面的 content-type 属性。
     * @return 返回值为“真”时，说明内容确实是 css 页面。
     */
    public static boolean isCssFile(String contentType)
    {
        return (contentType != null && !contentType.isEmpty())
                && "text/css".equals(contentType.trim().toLowerCase());
    }

    /**
     * Registers MIME type for provided extension.
     * Existing extension type will be overriden.
     */
    ...
    public static Set<String> lookupExtension(String contentType)
    {
        String ctty = contentType.toLowerCase();
        return mimeTypeMapping.entrySet().stream()
            .filter(a->a.getValue().equals(ctty))
            .map(a->a.getKey())
            .collect(Collectors.toSet());
    }

    public static String mjXtnsnOf(String contentType)
    {
        Set<String> exts = lookupExtension(contentType);
